<template>
  <div class="container" style="margin-top: 50px;">
    ...
    <el-card :body-style="{ padding: '0px' }" class="product-card">
        ...
        <h3 style="text-align:center;">画像 {{ item }}/4</h3>
        </el-carousel-item>
      </el-carousel>
    ...
    <el-card :body-style="{ padding: '0px' }" class="product-card">
        ...
        <el-scrollbar max-height="40px">
          <p class="comment" v-for="comment in comments" v-bind:key="comment">
            <el-avatar icon="el-icon-user-solid"></el-avatar>{{ comment }}</p>
        </el-scrollbar>
...
table th:after{ /*thの擬似要素のafter*/
  display: block;
  width: 0px;
  height: 0px;
  position: absolute;
  top:calc(50% - 10px); /*calc()，相対単位（%やvw, vhなど）と絶対単位（pxなど）を組み合わせる*/
  right:-10px;
